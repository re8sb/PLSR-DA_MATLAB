function [rho,pval] = correlatesHeatmap(full_X_data,sub_X_data,varNames_old,varNames)
%This function plots co-correlates heatmap when LASSO is used for feature
%selection.

[rho,pval]=corr(full_X_data, sub_X_data,'type','Spearman');

% rho = rho((length(full_X_data)+1):end,1:length(full_X_data)); 
rho(isnan(rho))=0; rho(rho==1)=0;
% rho_pval = pval((length(full_X_data)+1):end,1:length(full_X_data));
% rho(rho_pval>0.05)=0;

%subset to just significant correlations
rowlabels= varNames_old; %collabels = collabels(any(rho));
collabels = varNames; %rowlabels = rowlabels(any(rho,2));
