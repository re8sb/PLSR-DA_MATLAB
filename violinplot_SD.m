function violins = violinplot_SD(data, cats, varargin)
...
% This code is released under the terms of the BSD 3-clause license
    ...
    elseif hascategories && numel(data) == numel(cats)
        cats = categorical(cats,'Ordinal',true);
        catnames = categories(cats);
        for n=1:length(catnames)
            thisCat = catnames{n};
            thisData = data(cats == thisCat);
            violins(n) = Violin(thisData, n,'ViolinColor',varargin{2}(n,:)); %varargin{2} varargin{2}(5,:) varargin{:}
        end
        set(gca, 'xtick', 1:length(catnames), 'xticklabels', catnames);
        xtickangle(45)
    ...
    elseif ismatrix(data)
        for n=1:size(data, 2)
            thisData = data(:, n);
            violins(n) = Violin(thisData, n, 'ViolinColor',varargin{2}(n,:)); %varargin{:}
        end
        set(gca, 'xtick', 1:size(data, 2));
        if hascategories && length(cats) == size(data, 2)
            set(gca, 'xticklabels', cats,'TickLabelInterpreter','none');
        end
